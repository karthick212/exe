...
loadConductorMapping(user,callback) {
  ...
  let insertQuery = "select * from tblconductor where isActive<>'0' and Cid not in (select cid from tblconductorsuspend where isActive<>'0' and CurDate() between FromDate and ToDate)"
  return dbconfig.query(insertQuery,param, (err, results) => {
    if(err){
     return callback(null, err)
   }
