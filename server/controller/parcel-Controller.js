          ...
          dbconfig.query(dqry,[flat,flong,flat],(err,rows)=>{      
             ...
             distance.get({origin:flat+','+flong,destination:tolat+','+tolong },(err,rowss)=>{
              ...
              if(long>0)
              {
                if(parcelcat.indexOf('shar')>=0)
                {
                  let query="select offerprice,SDispRate from tbl_sharingrate where ServiceArea like ? and SharingSize='L-Large' and isActive!='0'"
                  dbconfig.query(query, ['%'+tocity+'%'], (err, rows1) => { try{ this.DistCalc(rows1[0].offerprice,rows1[0].SDispRate,dist,(res)=>{
                ...
                if(parceltype.indexOf('loc')!=0)
                {
                  let query="select offerprice from tbl_parcelrate where ParcelName=? and ParcelSize='L-Large' and isActive!='0' and FromCity like ? and ToCity like ?"
...
AddBooking(user,callback) {
 ...
 return callback(null, ResMsg)
  ...
  return callback(null, results)
      ...
      checkSharepass(code,cb){
        let qry='select * from vw_sharepassbalance where sharepasscode=?';
        dbconfig.query(qry,[code],(err,results) => {
      //console.log(results);
      return cb(results)
    })
      },
